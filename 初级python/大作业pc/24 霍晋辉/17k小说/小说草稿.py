...
for i in range(7, 81):
    ...
    headers = {
        ...
        'cookie': 'GUID=b02dea38-ee1f-44bf-bded-3d640d698ef0; UM_distinctid=174299cfbbb8c-01d8a844548788-5a472316-1fa400-174299cfbbe86; sensorsdata2015jssdkcross=%7B%22distinct_id%22%3A%22b02dea38-ee1f-44bf-bded-3d640d698ef0%22%2C%22%24device_id%22%3A%22174299cfc601c9-045cfefcbc300d-5a472316-2073600-174299cfc61d5%22%2C%22props%22%3A%7B%22%24latest_traffic_source_type%22%3A%22%E7%9B%B4%E6%8E%A5%E6%B5%81%E9%87%8F%22%2C%22%24latest_referrer%22%3A%22%22%2C%22%24latest_referrer_host%22%3A%22%22%2C%22%24latest_search_keyword%22%3A%22%E6%9C%AA%E5%8F%96%E5%88%B0%E5%80%BC_%E7%9B%B4%E6%8E%A5%E6%89%93%E5%BC%80%22%7D%7D; Hm_lvt_9793f42b498361373512340937deb2a0=1598425988,1598496180; CNZZDATA5647345=cnzz_eid%3D965041889-1598423450-https%253A%252F%252Fwww.baidu.com%252F%26ntime%3D1598497803; Hm_lpvt_9793f42b498361373512340937deb2a0=1598498503'
    }
    response = requests.get(url, headers=headers)
    response.encoding = 'utf-8'
    with open('小说草稿.html', 'w', encoding='utf-8') as file:
        file.write(response.text)
    print(response.text)
    html = etree.HTML(response.text)
    website = html.xpath('/html/body/div[4]/div[3]/div[2]/table/tbody/tr/td[3]/span/a/@href')
    print(website)
    for j in website:
        url = 'https:{}'.format(j)
        headers = {
        ...
        rederse = html.xpath('//div[@class="BookData"]//p/em[@id="howmuchreadBook"]/text()')[0]
        word = html.xpath('//div[@class="BookData"]//p/em[@class="red"]/text()')[0]
        update_day = html.xpath('//div[@class="BookData"]//p/em[@class="green"]/text()')
        if len(update_day) == 0:
            update_day = '目前在拼命写作中'
        else:
            update_day = update_day[0]
        dic = {
            '书名': book_name,
            '授权级别': level,
            '作品类别': book_type,
        ...
        # csv持久化
        header = ['书名', '授权级别', '作品类别', '本周点击', '本月点击', '本周更新', '本月更新', '周推荐票', '月推荐票', '人次', '字数', '更新天数']
        ...
        dic['书名'], dic['授权级别'], dic['作品类别'], dic['本周点击'], dic['本月点击'], dic['本周更新'], dic['本月更新'], dic['周推荐票'], dic['月推荐票'],
        dic['人次'], dic['字数'], dic['更新天数'])
        cursor.execute(sql)
